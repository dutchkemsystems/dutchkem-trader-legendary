import os
"""
Stability Analysis: Find the most consistent profitable configuration.
Run the best config 10x with fresh signals each time to measure variance.
"""
...
def get_signals(all_data):
    ...
    for sym in all_data:
        ...
        d = buy + sell
        if d == 0: signals[sym] = ("HOLD", 0.0)
        elif buy > sell: signals[sym] = ("BUY", d / max(len(valid), 1))
        elif sell > buy: signals[sym] = ("SELL", d / max(len(valid), 1))
        else: signals[sym] = ("HOLD", 0.0)
    return signals


def backtest(all_data, signals, cfg):
    kelly = KellySizer()
    ...
    for sym, df in all_data.items():
        ...
        total_pnl += sum(trades); total_trades += len(trades); wins += sw
        per_sym[sym] = {"trades": len(trades), "pnl": sum(trades)}
    return total_pnl, total_trades, wins, per_sym


def main():
    print("=" * 65)
    print("  STABILITY ANALYSIS: Multiple signal generations per config")
    print("=" * 65)
    ...
    configs = [
        ("Risk 25%, CB3, no Tue", {"max_risk": 0.25, "circuit_breaker": 3, "bad_hours": {19, 22}, "bad_days": set()}),
        ("Risk 25%, CB3, Tue", {"max_risk": 0.25, "circuit_breaker": 3, "bad_hours": {19, 22}, "bad_days": {1}}),
        ("Risk 30%, CB3, Tue", {"max_risk": 0.30, "circuit_breaker": 3, "bad_hours": {19, 22}, "bad_days": {1}}),
        ("Risk 30%, CB4, Tue", {"max_risk": 0.30, "circuit_breaker": 4, "bad_hours": {19, 22}, "bad_days": {1}}),
        ("Risk 28%, CB3, Tue", {"max_risk": 0.28, "circuit_breaker": 3, "bad_hours": {19, 22}, "bad_days": {1}}),
    ]
    ...
    for cfg_name, cfg in configs:
        ...
        for run in range(5):
            ...
            pnl, trades, wr, per_sym = backtest(all_data, signals, cfg)
            pnls.append(pnl)
            active = sum(1 for a, g in signals.values() if a != "HOLD" and g >= 0.25)
            print(f"    Run {run+1}: {trades:4} trades | P&L=${pnl:+.2f} | {active} active symbols")
        avg = sum(pnls) / len(pnls)
        print(f"    AVG: ${avg:+.2f} | MIN: ${min(pnls):+.2f} | MAX: ${max(pnls):+.2f}")
        above_50 = sum(1 for p in pnls if p >= 50)
        print(f"    Above $50: {above_50}/{len(pnls)}")
        print()
